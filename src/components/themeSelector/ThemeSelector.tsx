...
const ThemeSelector: React.FC<ThemeSelectorProps> = ({ isOpen, onClose, closeOnBackdropClick }) => {
  ...
  const ThemeCircle = ({ theme, isOfficial = false }: { theme: Theme; isOfficial?: boolean }) => (
    <button
      ...
      className={`relative w-16 h-16 aspect-square flex items-center justify-center rounded-full transition-all duration-200 focus:outline-none box-border ${
        currentTheme.name === theme.name ? 'border-[2.6px] shadow-lg' : ''
      }`}
      style={{ 
